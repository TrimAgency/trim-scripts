...
curl --request PUT -u "$username:$token" https://api.github.com/teams/1238077/repos/trimagency/"$repo_name" -d '{"permission":"admin"}' > /dev/null 2>&1
echo " done."

echo -n "Adding TeamTrim team to Github repository '$repo_name' ..."
curl --request PUT -u "$username:$token" https://api.github.com/teams/1975081/repos/trimagency/"$repo_name" -d '{"permission":"admin"}' > /dev/null 2>&1
echo " done."
